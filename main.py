...
def check_multiple_combinations(populations=[],seletion_strategies=[],generation=10,path=""):
    ...
    for po in populations:
        for strategy in seletion_strategies:
            env_space = gym.make('LunarLander-v2')
            env_space.reset()
...
class GeneticAlgorithm:
    ...
    def to_weights(self):
        ...
        # the expression  np.sqrt(2 / self.input_layer) is a commonly  used heuristic for initializing weights in the input layer of a neural network to promote more stable and effective trainin
    ...
    def rr_wheel_selection(self,gene_rewards, num_selections):
        total_reward = sum(gene_rewards)
        probabilities = [rd / total_reward for rd in gene_rewards]
        ...
        for _ in range(num_selections):
            ...
            # Select an index based on the roulette wheel
    ...
    def to_evolve(self, score, gene,population,parent_pop,strategy="greedy"):
        n = random.randint(0, len(score))
        parents = []
        parent_pop = random.randint(10, population)
        if strategy == "greedy":
            ...
            selection=parent_pop
            selection_indices=self.rr_wheel_selection(gene_rewards=score,num_selections=selection)
            for index in selection_indices:
                if index< len(gene) and index<len(score):
                    score.remove(score[index])
                    parents.append(gene[index].reshape(1, -1))
                    del gene[index]
    ...
    def init_population(self, population, iteration_time,is_model_loaded=False,load_model_weights=(1,1,1)):
        ...
        for _ in range(population):
            ...
            for __ in range(iteration_time):
                # env_space.render()
                model = DeepNeuralNetwork(8, 36, 36, 4)
...
class Agent():
    ...
    def optimize_agent(self,weight_path=""):
        ...
        while True:
            new_population = self.ga_model.to_evolve(self.pop_award, self.pop_gene,population=self.population,parent_pop=self.parent_pop,strategy=self.strategy)
            self.pop_award = []
            self.pop_gene = []
